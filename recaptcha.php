...
REGISTRATION;
        ...
        function validate_options($input) {
            ...
            $themes = array ('red', 'white', 'blackglass', 'clean', 'custom');
...
COMMENT_FORM;
		...
		if ($this->options['comments_theme'] == 'custom') {
                    $recaptcha_js_opts = <<<OPTS
                        <script type='text/javascript'>
                            var RecaptchaOptions = { theme : '{$this->options['comments_theme']}', custom_theme_widget : 'recaptcha_widget', lang : '{$this->options['recaptcha_language']}' , tabindex : {$this->options['comments_tab_index']} };
                        </script>
OPTS;
		}
		else {
...
COMMENT_FORM;
                ...
                //Rush@20140717-----------
                //Forms for custom theme
                $recaptcha_custom_form = <<<CUSTOM_FORM
...
//Rush@20140717-----------
                ...
                if ($this->options['comments_theme'] == 'custom') {
                    echo $recaptcha_js_opts . $recaptcha_custom_form . $this->get_recaptcha_html(isset($escaped_error) ? $escaped_error : null, $use_ssl) . $comment_string;
                } else {
...
JS;
		...
		__('Custom', 'recaptcha') => 'custom'	//@20140715_byRush: add custom options to customize the Look and Feel of reCaptcha
            );
